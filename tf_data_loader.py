...
def load_dataset_multi(root, image_size, seq_len, shift, stride, label_scale):
    ...
    for (run_number, d) in tqdm(enumerate(dirs)):
        labels = np.genfromtxt(os.path.join('Test1/data_out.csv'), delimiter=',', skip_header=1, dtype=np.float32)
        ...
        for ix in range(n_images):
            ...
            s = 'Image'+str(ix+1)+'.png'
            img = Image.open(os.path.join(root, s))
...
training_dataset, validation_dataset = get_dataset_multi('Test1', IMAGE_SHAPE, 1030,
                                                                     data_shift,
